...
class zeroday(Cog):
    ...
    async def inviteZeroday(self, ctx):
        ...
        embedInviteBot = discord.Embed(
            ...
            value="[Invite Link](https://discord.com/api/oauth2/authorize?client_id=604882921958146063&permissions=574475584&scope=bot)",
        )
        await ctx.send(ctx.author.mention, embed=embedInviteBot)
    ...
    async def help(self, context, *module):
        ...
        if not module:
            ...
            content.add_field(
                name="Extension",
                value=module_description[0 : len(module_description) - 1],
                inline=False,
            )
            ...
            if len(module) > 1:
                ...
                for x in self.bot.cogs:
                    for y in module:
                        if x == y:
                            content = discord.Embed(
                                title=module[0] + " 명령어 목록",
                                description=self.bot.cogs[module[0]].__doc__,
                            )
                            for z in self.bot.get_cog(y).get_commands():
                                if not z.hidden:
                                    content.add_field(
                                        name=z.name, value=z.help, inline=False
                                    )
                            found = True
                if not found:
                    content = discord.Embed(
