...
class Status(Cog):
    ...
    async def _system(self, ctx):
        ...
        source.add_field(name="Architecture", value=str(platform.architecture()[0]), inline=False)
        ...
        source.add_field(name="System version", value=str(platform.version()), inline=False)
        ...
        source.add_field(name="CPU utilization", value=str(cpu_percent) +" %", inline=False)
        source.add_field(name="CPU frequency", value=str(cpu_freq) + " GHz", inline=False)
        source.add_field(name="RAM capacity", value=str(total) + " GB", inline=False)
        source.add_field(name="RAM utilization", value=str(available_mem) + " GB", inline=False)
